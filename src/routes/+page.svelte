<script lang="ts">
  ...
  type QualityMode = "quality_first" | "compatibility_first";
  type OutputCodec = "auto" | "vp9" | "h264";
  type DownloadItem = { id: string; url: string; title: string; progress: number; speed: string; eta: string; status: Status; phase: "" | "downloading" | "downloaded" | "converting" | "encoded"; phase_progress: number; error: string | null; error_log: string[]; output_dir: string; profile: string; profile_args: string[]; created_at: number; updated_at: number };
  type Settings = { max_concurrent_downloads: number; default_folder: string; default_profile: string; resume_on_launch: boolean; profile_args: string[]; theme: Theme; quality_mode: QualityMode; output_codec: OutputCodec; transcode_if_needed: boolean };
  type AppState = { settings: Settings; items: DownloadItem[]; yt_dlp_path: string | null; ffmpeg_path: string | null; deno_path: string | null };

  let appState: AppState = { settings: { max_concurrent_downloads: 3, default_folder: "~/Downloads/yt-dlp", default_profile: "Default", resume_on_launch: true, profile_args: [], theme: "auto", quality_mode: "quality_first", output_codec: "h264", transcode_if_needed: true }, items: [], yt_dlp_path: null, ffmpeg_path: null, deno_path: null };
  let urlInput = "";
  ...
  type CodecPreset = "quality_h264" | "quality_vp9" | "compatibility" | "custom";
  function selectedCodecPreset(): CodecPreset {
    if (settingsDraft.quality_mode === "quality_first" && settingsDraft.output_codec === "h264" && settingsDraft.transcode_if_needed) return "quality_h264";
    if (settingsDraft.quality_mode === "quality_first" && settingsDraft.output_codec === "vp9" && !settingsDraft.transcode_if_needed) return "quality_vp9";
    if (settingsDraft.quality_mode === "compatibility_first" && settingsDraft.output_codec === "h264" && !settingsDraft.transcode_if_needed) return "compatibility";
    return "custom";
  }
  function applyCodecPreset(preset: CodecPreset) {
    if (preset === "quality_h264") settingsDraft = { ...settingsDraft, quality_mode: "quality_first", output_codec: "h264", transcode_if_needed: true };
    if (preset === "quality_vp9") settingsDraft = { ...settingsDraft, quality_mode: "quality_first", output_codec: "vp9", transcode_if_needed: false };
    if (preset === "compatibility") settingsDraft = { ...settingsDraft, quality_mode: "compatibility_first", output_codec: "h264", transcode_if_needed: false };
  }
  ...
  function progressLabel(item: DownloadItem) {
    if (item.phase === "downloaded") return "ダウンロード完了 · 100%";
    if (item.phase === "converting") return `エンコード中 · ${item.phase_progress.toFixed(0)}%`;
    if (item.phase === "encoded") return "エンコード完了 · 100%";
    if (item.status === "paused") return `一時停止 · ${item.phase_progress.toFixed(0)}%`;
    return `ダウンロード中 · ${item.phase_progress.toFixed(0)}%`;
  }
  ...
  {#if isLoading}<div class="empty-state">読み込み中…</div>{:else}
    <section class="section downloads-section"><div class="section-heading"><div><h2>ダウンロード</h2><span class="subheading">進行中・待機中</span></div><div class="queue-heading-actions"><span class="queue-count-control" class:has-items={queueItems.length > 0}><span class="count queue-count">{activeItems.length + queueItems.length}</span>{#if queueItems.length > 0}<button class="clear-queue-button" aria-label="待機中のキューをクリア" title="待機中のキューをクリア" onclick={clearQueue}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 7h12m-9 0V5h6v2m-8 0 1 13h8l1-13M10 10v7m4-7v7" /></svg></button>{/if}</span></div></div>
      {#if activeItems.length === 0 && queueItems.length === 0}<div class="empty-state compact">ダウンロードはありません</div>{:else}<div class="download-list">
        {#each activeItems as item (item.id)}<article class="download-card" class:paused={item.status === "paused"} class:downloaded={item.phase === "downloaded"} class:converting={item.phase === "converting"} class:encoded={item.phase === "encoded"} style={`--progress: ${Math.min(100, Math.max(0, item.progress))}%`}><div class="download-progress-fill" aria-hidden="true"></div><div class="card-top"><div class="download-symbol">↓</div><div class="item-info"><strong title={item.title}>{shortTitle(item)}</strong><span><b class="progress-percent">{progressLabel(item)}</b></span></div></div></article>{/each}
...
{#if showSettings}
  <div class="modal-backdrop" role="presentation" onclick={(event) => event.target === event.currentTarget && closeSettings()}>
    <dialog open class="modal" aria-labelledby="settings-title">
      ...
      <div class="setting-field codec-policy"><div class="setting-label-row"><span class="setting-label">画質とコーデック</span><button type="button" class="info-tip" aria-label="画質とコーデックの説明"><span class="info-glyph">i</span><span class="tooltip-bubble" role="tooltip">画質基準とコーデックを別々に選べます。変換を有効にすると、指定コーデックがない場合にffmpegで変換します。</span></button></div><div class="codec-presets"><button type="button" class:chosen={selectedCodecPreset() === "quality_h264"} onclick={() => applyCodecPreset("quality_h264")}><strong>最高画質をH.264化</strong><small>最大解像度を取得して互換化</small></button><button type="button" class:chosen={selectedCodecPreset() === "quality_vp9"} onclick={() => applyCodecPreset("quality_vp9")}><strong>VP9をそのまま保存</strong><small>変換せず画質を優先</small></button><button type="button" class:chosen={selectedCodecPreset() === "compatibility"} onclick={() => applyCodecPreset("compatibility")}><strong>互換性優先・最大解像度</strong><small>H.264を優先して取得</small></button></div><div class="codec-matrix"><div class="matrix-choice"><span>画質基準</span><label><input type="radio" name="quality-mode" bind:group={settingsDraft.quality_mode} value="quality_first" />画質優先</label><label><input type="radio" name="quality-mode" bind:group={settingsDraft.quality_mode} value="compatibility_first" />互換性優先</label></div><div class="matrix-choice"><span>コーデック</span><label><input type="radio" name="output-codec" bind:group={settingsDraft.output_codec} value="auto" />自動</label><label><input type="radio" name="output-codec" bind:group={settingsDraft.output_codec} value="vp9" />VP9</label><label><input type="radio" name="output-codec" bind:group={settingsDraft.output_codec} value="h264" />H.264</label></div><label class="check-row"><input type="checkbox" bind:checked={settingsDraft.transcode_if_needed} /> 指定がなければ変換する</label></div><small>「自動」は取得元の形式を尊重します。変換を有効にすると、指定コーデックへ揃えます。</small></div>
...
<style>
  ...
  :global(body.theme-dark) .drop-zone p,:global(body.theme-dark) .brand p,:global(body.theme-dark) .item-info span,:global(body.theme-dark) .queue-title span,:global(body.theme-dark) .subheading,:global(body.theme-dark) .drop-zone small,:global(body.theme-dark) .modal-heading p{color:#aaa4b0}
  ...
  :global(body.theme-dark) .codec-presets button{border-color:#4c4654;background:#2b2830;color:#eeeaf2}:global(body.theme-dark) .codec-presets button:hover{border-color:#8063b4;background:#352d40}:global(body.theme-dark) .codec-presets button.chosen{border-color:#8063b4;background:#3a304d;box-shadow:0 0 0 2px #b49add33}:global(body.theme-dark) .codec-presets small{color:#aaa4b0}:global(body.theme-dark) .codec-matrix label:not(.check-row){color:#eeeaf2}
  ...
  .download-list{display:grid;gap:8px}.download-list .queue-row{background:#fff;border:1px solid #ebe7ef;border-radius:16px}.download-list .queue-row:last-child{border-bottom:1px solid #ebe7ef}.download-card{position:relative;overflow:hidden}.download-card>*:not(.download-progress-fill){position:relative;z-index:1}.download-progress-fill{position:absolute;inset:0 auto 0 0;z-index:0;width:var(--progress);background:#d9cdf5;opacity:.55;transition:width .2s linear;pointer-events:none}.download-card.paused .download-progress-fill{background:#cfc2e9}.download-card.converting .download-progress-fill{background:#cbbbec}.download-card.downloaded .download-progress-fill,.download-card.encoded .download-progress-fill{background:#c5dfce;opacity:.65}.progress-percent{color:#6750a4;font-size:11px}
  :global(body.theme-dark) .download-list .queue-row{background:#211f25;border-color:#45404c}:global(body.theme-dark) .download-list .queue-row:last-child{border-bottom-color:#45404c}:global(body.theme-dark) .download-progress-fill{background:#5f4e82;opacity:.55}:global(body.theme-dark) .download-card.paused .download-progress-fill{background:#514465}:global(body.theme-dark) .download-card.converting .download-progress-fill{background:#6b5792}:global(body.theme-dark) .download-card.downloaded .download-progress-fill,:global(body.theme-dark) .download-card.encoded .download-progress-fill{background:#41654d}:global(body.theme-dark) .progress-percent{color:#d3baff}
  .matrix-choice{display:flex;align-items:center;gap:6px;flex-wrap:wrap}.matrix-choice>span{width:100%;color:#55505c;font-size:11px;font-weight:650}.matrix-choice label{display:flex;align-items:center;gap:5px;padding:7px 9px;border:1px solid #dfdbe4;border-radius:9px;color:#625c69;background:#fbfafc;font-size:11px;font-weight:400;cursor:pointer}.matrix-choice label:hover{border-color:#bba9df;background:#f6f2fc}.matrix-choice input{width:14px;height:14px;margin:0;accent-color:#6750a4}.matrix-choice label:has(input:checked){border-color:#8b6ac2;background:#f0eaff;color:#51406f;font-weight:650}
  :global(body.theme-dark) .matrix-choice>span{color:#eeeaf2}:global(body.theme-dark) .matrix-choice label{border-color:#4c4654;color:#c0bac5;background:#2b2830}:global(body.theme-dark) .matrix-choice label:hover{border-color:#8063b4;background:#352d40}:global(body.theme-dark) .matrix-choice label:has(input:checked){border-color:#8063b4;background:#3a304d;color:#eeeaf2}
  .codec-matrix .matrix-choice{min-width:0;align-content:start}.codec-matrix .matrix-choice>label{display:flex;align-items:center;justify-content:flex-start;gap:5px;min-width:0;white-space:nowrap}.codec-matrix .matrix-choice>label input{flex:0 0 auto}
  .codec-matrix .matrix-choice{display:grid;grid-template-columns:minmax(0,1fr);gap:7px;align-content:start}.codec-matrix .matrix-choice>span{width:auto}.codec-matrix .matrix-choice>label{display:flex!important;flex-direction:row!important;align-items:center!important;justify-content:flex-start!important;width:100%!important;min-width:0!important;min-height:34px!important;padding:7px 10px!important;white-space:normal!important}.codec-matrix .matrix-choice>label input{display:block!important;flex:0 0 auto!important;width:14px!important;height:14px!important}
</style>
